...
# icon for application when opened
root.iconbitmap(r'C:\Users\Потребител\Desktop\REPO\Personal-projects\Downloader\2.1\logo.ico')

# color background for the application
...
def data_mp4():
    global chosen_type
    chosen_type = 'MP4'
    data_type = tk.Label(root, text=f"Current Data Type: {chosen_type}", font=("Arial", 10), background='yellow')
...
title_label = tk.Label(root,
                       text="Please enter link below to download",
                       font=("Arial", 13), foreground='black',
                       width=40,
                       background='yellow',
                       )
...
data_type = tk.Label(root, text=f"Current Data Type:{chosen_type}", font=("Arial", 10), background='yellow', foreground='black')
data_type.grid(row=4, column=1)
